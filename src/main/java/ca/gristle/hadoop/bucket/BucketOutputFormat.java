...
public class BucketOutputFormat extends FileOutputFormat<Text, BytesWritable> {
    public static Logger LOG = LoggerFactory.getLogger(BucketOutputFormat.class);
    public static final String SPEC_ARG = "bucket_spec_arg";
    ...
    /**
     * Change this to just use Bucket#writeObject - automatically fix up BytesWritable
     */
    public static class BucketRecordWriter implements RecordWriter<Text, BytesWritable> {
        ...
        public BucketRecordWriter(JobConf conf, String unique, Progressable p) throws IOException {
            BucketSpec spec = (BucketSpec) Utils.getObject(conf, SPEC_ARG);

            Path path = getOutputPath(conf);
            FileSystem fs = path.getFileSystem(conf);

            Bucket.create(fs, path.toString(), spec,  false);
            ...
            _bucket = Bucket.create(fs, FileOutputFormat.getTaskOutputPath(conf, unique).getParent().toString(), spec, false);
            _unique = unique;
        }
        ...
        public void write(Text k, BytesWritable v) throws IOException {
            ...
            if(oaf==null) {
                ...
                oaf = new OpenAttributeFile(attr, filename, _bucket.openWrite(filename, true));
    ...
    public RecordWriter<Text, BytesWritable> getRecordWriter(FileSystem ignored, JobConf jc, String string, Progressable p) throws IOException {
        return new BucketRecordWriter(jc, string, p);
    }
    ...
    public void checkOutputSpecs(FileSystem fs, JobConf conf) throws IOException {
        ...
        if(!(conf.getOutputCommitter() instanceof FileOutputCommitter)) {
            if(conf.getMapSpeculativeExecution() && conf.getNumReduceTasks()==0 || conf.getReduceSpeculativeExecution()) {
                throw new IllegalArgumentException("Cannot use speculative execution with BucketOutputFormat unless FileOutputCommitter is enabled");
            }
        }
    }
